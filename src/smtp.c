		...
		param_list_t *defparm)
{
	...
	*tok++ = 0;			/* split name off */
    ...
    if (!pish->filePattern) {
	D_PRINTF(stderr,"missing file (or generator) for SMTP command");
	return returnerr(stderr,"missing file (or generator) for SMTP command\n");
    }

    SmtpFilePrep (cmd, defparm);

// comment this all out fow now - this needs to be fixed though if AUTOGEN
// is to work
// Thom O'Connor, 2005/12
//   #ifdef AUTOGEN
...
//        /* read the contents of file into struct */
//        memset(&statbuf, 0, sizeof(statbuf));
//        if (stat(pish->filePattern, &statbuf) != 0) {
//           return returnerr(stderr,"Couldn't stat file %s: errno=%d: %s\n",
//                            pish->filePattern, errno, strerror(errno));
//        }
//
//        /* open file */
//        if ((fd = open(pish->filePattern, O_RDONLY)) <= 0) {
//            return returnerr(stderr, "Cannot open file %s: errno=%d: %s\n",
//                             pish->filePattern, errno, strerror(errno));
//       }
//
...
//           returnerr(stderr, "Error reading file %s, got %d expected %d\n",
//                     pish->filePattern, bytesRead, pish->msgsize);
//           close(fd);
//           return -1;
//       }
//
...
extern double d_millitime_atoi(const char*);
		    ...
		    char *tok)
{
	...
	pish->numRecipients = parse_distrib(tok, &atof);
    else if (strcmp(name, "file") == 0)
	pish->filePattern = xstrdup (tok);
    ...
    /* Sean O'Rourke: parse Multi-pop attr's */
      ...
      pish->userSpacing
         = parse_distrib(tok, &d_millitime_atoi);
    else if (strcmp(name, "droprate") == 0)
      pish->dropRate = parse_distrib(tok, &atof);
  ...
  # endif /* GEN_CHECKSUM */
        ...
        pish->bandwidth = parse_distrib(tok, &size_atof);
...
{
    ProtocolRegister(
        "SMTP", SmtpParseStart, SmtpParseEnd,
        sendSMTPStart, sendSMTPLoop, sendSMTPEnd,
